...
public class DeclareScuttageVassalAction : Action
{
	public DeclareScuttageVassalAction(Character owner, Def def)
		: base(owner, def)
	{
	}
	...
	{
		return new DeclareScuttageVassalAction(owner as Character, def);
	}
	...
	public override List<Value>[] GetPossibleArgs()
	{
		...
		if (args == null)
		{
			...
			for (int i = 0; i < own_kingdom.vassalStates.Count; i++)
			{
				...
				if (kingdom.IsVassal() && (kingdom.vassalage == null || kingdom.vassalage.def.type != Vassalage.Type.Scuttage))
				{
					list.Add(kingdom);
				}
			}
	...
	public override void Run()
	{
		...
		{
			args[0].Get<Kingdom>().ChangeVassalageType(Vassalage.Type.Scuttage);
		}
	}
}
