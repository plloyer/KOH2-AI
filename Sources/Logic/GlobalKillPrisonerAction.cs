...
public class GlobalKillPrisonerAction : Action
{
	...
	{
		return new GlobalKillPrisonerAction(owner as Kingdom, def);
	}
	...
	{
		if (!(own_kingdom.actions.Find("KillPrisonerAction") is KillPrisonerAction))
		{
			return;
		}
		for (int num = own_kingdom.prisoners.Count - 1; num >= 0; num--)
		{
			...
			if (character != null && character.IsValid())
			{
				...
				if (kingdom != own_kingdom)
				{
					...
					own_kingdom.AddRelationModifier(kingdom, "rel_executed_prisoner", null, own_kingdom.royal_dungeon.GetVar("relationship_execute", vars));
				}
			}
		}
		if (prisonersPerKingdom != null)
		{
			foreach (KeyValuePair<Kingdom, List<Character>> item in prisonersPerKingdom)
			{
				...
				key.FireEvent("prisoners_executed", value, key.id);
			}
		}
		base.Run();
	}
