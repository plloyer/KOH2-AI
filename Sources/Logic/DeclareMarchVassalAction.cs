...
public class DeclareMarchVassalAction : Action
{
	public DeclareMarchVassalAction(Character owner, Def def)
		: base(owner, def)
	{
	}
	...
	{
		return new DeclareMarchVassalAction(owner as Character, def);
	}
	...
	public override List<Value>[] GetPossibleArgs()
	{
		...
		if (args == null)
		{
			...
			for (int i = 0; i < own_kingdom.vassalStates.Count; i++)
			{
				...
				if (kingdom.IsVassal() && (kingdom.vassalage == null || kingdom.vassalage.def.type != Vassalage.Type.March))
				{
					list.Add(kingdom);
				}
			}
	...
	public override void Run()
	{
		...
		{
			args[0].Get<Kingdom>().ChangeVassalageType(Vassalage.Type.March);
		}
	}
}
